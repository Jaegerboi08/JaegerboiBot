...
class Bot(BotCommandAgent):
    ...
    def run(self):
        ...
        if self.get_intent() is not None:
            return
        d1 = (abs(self.ball.location.y - self.foe_goal.location.y))+100
        ...
        if self.kickoff_flag:
            ...
            return
        if abs(self.foes[0].location.x - self.ball.location.x) < 500 and abs(self.foes[0].location.y - self.ball.location.y) < 500 and d1<1200:
            self.set_intent(goto(self.friend_goal.location))
            if 700>self.ball.location.x>300 and d2>2000:
                self.set_intent(atba())
                return   
        if 100<d2<600 and in_front == False:
